...
public class UserDTO extends EntityDTO {
    ...
    @EntityField
    @NotBlank(message = "username不能为空", groups = {AddGroup.class, UpdateGroup.class})
    @Pattern(regexp = "^[a-zA-Z][a-zA-Z0-9]{7,15}$", message = "8-16位字母+数字组合，必须英文开始")
    private String username;

    @EntityField
    @NotBlank(message = "phone不能为空", groups = {AddGroup.class, UpdateGroup.class})
    @Pattern(regexp = "^[1-9]\\d{10}$", message = "手机号码格式有误")
    private String phone;

    @EntityField
    @NotBlank(message = "nickname不能为空", groups = {AddGroup.class, UpdateGroup.class})
    ...
    @NotEmpty(message = "部门ids不能为空", groups = {AddGroup.class, UpdateGroup.class})
    ...
    @NotEmpty(message = "角色ids不能为空", groups = {AddGroup.class, UpdateGroup.class})
